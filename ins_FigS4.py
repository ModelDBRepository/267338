...
for factor in [1,2,3,5]:
	...
	rateses = [ [ np.loadtxt("RatesFig7/NonPhysrates_Esm%d_cm%d_%dHzgChR%d_Fact%d.dat" % (int(np.round(-Es)),kcm,int(fsin),int(gsin), int(factor))) for kcm in range(Nnets) ] for Es in Ess ];
	...
	for kEs in range(len(Ess)):
		...
		tmp = plt.errorbar(range(Nnets), np.mean(maxPowses[kEs],axis=1), yerr=np. std(maxPowses[kEs],axis=1), marker="o",color=color[factor-1], ls="");
	...
	tmp = plt.errorbar(range(len(meansfs_hyp)), meansfs_hyp, yerr=stdsfs_hyp, marker="o", color=color[factor-1], ls="");
	tmp = plt.ylim(0.,450.);
	tmp1.spines['top'].set_visible(False)
	tmp1.spines['right'].set_visible(False)
	tmp1.spines['bottom'].set_visible(True)
	...
	#tmp1.set_xticklabels([])
	#tmp1.set_yticks([0,200,400])
plt.legend(["x1","x2","x3","x5"])
plt.savefig("Figures/Supp4_%dHzgChR%dFactor%d.eps" % (int(fsin),int(gsin), int(factor)) );plt.savefig("Figures/Supp4_%dHzgChR%dFactor%d.png" % (int(fsin),int(gsin),int(factor)) );
