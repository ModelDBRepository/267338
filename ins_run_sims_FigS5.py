...
for gLmin in [1.,1.5,2.]:
    for Es in Ess:
       ...
       #for conn_mat in range(Nnets):
        ...
        rates_ds = rates[::kds];
        tmp = np.savetxt("RatesFig7/gLMin%d_Esm%d_cm%d_%dHzgChR%d_Fact%d.dat" % (int(glMin),int(np.round(-Es)),conn_mat,int(fsin),int(gsin), int(factor)), rates_ds );
